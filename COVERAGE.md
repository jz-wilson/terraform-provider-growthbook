...
| `scheduleRules` | `rules[].schedule_rules[]` (`enabled`, `timestamp`) | yes | Simple time-based on/off schedule; a plain (non-pointer) list, same replaced-wholesale reasoning as rule-level prerequisites. Requires GrowthBook Pro (`schedule-feature-flag`); on other plans GrowthBook drops it and the provider reports an error. `timestamp` is `timetypes.RFC3339`, not a plain string: GrowthBook's Mongo Date -> ISO round trip reformats a configured value (e.g. adding a `.000` fraction), and the custom type's semantic equality absorbs that instead of producing a diff. |
